  ...
  if (config.mcpWattioUrl) {
    ...
    parts.push(
      [
        ...
        "",
        "**Mapa dos índices (já fornecido — pule a descoberta).** Os índices e seus campos-chave já estão abaixo; **não chame `list_indices` nem `get_index_mapping`** a menos que precise de um campo que não esteja aqui. Para `term`/`sort` use o subcampo `.keyword`; para `match`, a raiz.",
        "",
        "- **`database_producao`** — os **chamados/tarefas** (espelho do Notion). Recupere sempre via `get_task_by_id`; o UUID da página fica em `id`/`_id`. O conteúdo vive em `properties.*` (ex.: título, descrição, `Status`, `Cliente`, `Categoria`, `Tipo`, `Prioridade`, responsável, `Anexos`, link `GITHUB`, código da task em `IDN8N`/`ID2`) — o objeto completo já volta no `get_task_by_id`.",
        "- **`notion_comments`** — comentários das tarefas; junção por `page_id.keyword` (UUID da página). Campos: `text` (conteúdo legível — reprodução, erros, cliente/módulo), `created_time`, `created_by_id`, `discussion_id`, `mentioned_user_names`; `raw` só quando `text` não bastar.",
        "- **`notion_attachments`** — anexos; junção por `page_id.keyword`. Campos: `attachment_name`, `attachment_type`, `attachment_url` (URL S3 assinada), `expiry_time`, e `raw.file.url`/`raw.external.url`.",
        "- **`pessoas`** — pessoas/times (Notion); conteúdo em `properties.*` (nome, time, id).",
        "- **`projetos`** — projetos (Notion); conteúdo em `properties.*` (nome, status, responsável, times, prioridade, produtos, tarefas).",
        "",
        ...
        "- **Comentários:** `run_opensearch_query` no índice `notion_comments`, junção pelo UUID da página: `{\"size\":30,\"query\":{\"terms\":{\"page_id.keyword\":[\"<task page UUID>\"]}},\"sort\":[{\"created_time\":{\"order\":\"asc\"}}]}`. Em cada comentário leia `text` (a reprodução, erros e cliente/módulo afetado costumam estar aí); recorra ao objeto `raw` só quando `text` não bastar. O servidor limita o nº de hits e trunca textos muito longos — se vier `\"truncated\":true`, refine a query em vez de pedir tudo.",
        "- **Anexos:** `run_opensearch_query` no índice `notion_attachments`, com todos os UUIDs coletados (a página costuma referenciar sub-páginas): `{\"size\":30,\"query\":{\"terms\":{\"page_id.keyword\":[\"<task page UUID>\",\"<related page UUID>\"]}}}`. Cada anexo tem `attachment_name`, `attachment_type`, `attachment_url` (URL S3 assinada) e `expiry_time`; abra com `WebFetch` apenas os **3–4 anexos mais relevantes** (priorize prints de erro/telas do fluxo). Considere também links inline citados em `raw` dos comentários.",
        "",
        ...
        "",
        "Diretrizes:",
        "- Os índices/campos já estão no mapa acima — use-os direto. Só recorra a `get_index_mapping` se precisar de um campo ausente do mapa; evite `list_indices`.",
  ...
  if (config.mcpPostgresUrl) {
    ...
    parts.push(
      [
        ...
        "",
        "**Mapa do banco (já fornecido — pule a varredura).** É um **Odoo** (schema `public`, ~637 tabelas). **Não rode `list_schemas`/`list_objects` para varrer**: sabendo a tabela, vá direto a `get_object_details` nela. Convenções Odoo: PK sempre `id`; auditoria em `create_uid`/`write_uid`/`create_date`/`write_date`; toda tabela `<a>_<b>_rel` é uma junção N:N (duas FKs). Domínio: SaaS de **Geração Distribuída (GD) de energia**. Tabelas-chave por área:",
        "- **Clientes/usuários:** `res_partner` (clientes/parceiros), `res_users` (usuários), `res_company` (empresas/unidades).",
        "- **Contratos & propostas:** `gd_contract`, `gd_document` (propostas/documentos), `gd_document_type`.",
        "- **Faturamento:** `invoice_client` (fatura do cliente), `bill` + `bill_line`/`bill_historical_line`, `bill_batch`, `gd_historical_invoice_client`.",
        "- **Energia (GD):** `gd_distribution`/`gd_distribution_line`, `gd_consolidation`/`gd_consolidation_line`, `gd_projection`, usinas (`*_usina_rel`).",
        "- **Comissões:** `gd_commission` (+ relacionados). **CRM:** `crm_lead`.",
        "- **Infra Odoo:** `ir_attachment` (anexos), `mail_message`/`mail_tracking_value` (e-mails/log), `queue_job` (jobs assíncronos).",
        "Para as colunas exatas de uma tabela, chame `get_object_details` só nela — não presuma nomes de coluna.",
        "",
        ...
        "",
        "Fluxo sugerido:",
        "1. **Confirme só as tabelas relevantes:** depois de identificar no código *quais* tabelas o fluxo manipula, use `get_object_details` direto nessas poucas tabelas (colunas, relacionamentos, índices). Recorra a `list_schemas`/`list_objects` apenas se ainda não souber o nome da tabela — não como varredura inicial padrão.",
        ...
        "",
        "Diretrizes (além do orçamento e das regras de leitura já definidos acima):",
        "- **Foco:** inspecione apenas as poucas tabelas que o fluxo do chamado realmente toca — nada de mapear o schema inteiro ou iterar tabela a tabela. Assim que tiver o dado que confirma o caso, **pare e escreva**.",
        "- Antes de montar um SELECT, **descubra o schema real** apenas das tabelas envolvidas (`get_object_details` na tabela específica) em vez de varrer o `list_objects` inteiro; não presuma nomes de tabelas/colunas — confirme só o que vai usar.",
