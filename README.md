...
1. Cliente faz `POST /enrich` (alias `POST /ask`) com `Authorization: Bearer <key>` e `{ "ticket": "Não consigo gerar a fatura desse cliente." }`.
...
├── prompts/
│   ├── system-base.md          # papel (Analista Sênior) + regras inegociáveis (somente leitura)
│   └── enriquecimento.md       # tarefa + formato de saída estruturado para o Notion
├── scripts/
...
└── src/
    ...
    ├── routes/ask.js           # endpoints POST /enrich e /ask (alias)
    ├── services/
    ...
    └── utils/
        ├── parseTicket.js      # normaliza o relato do chamado
...
`POST /enrich` (alias `POST /ask`) exige `Authorization: Bearer <key>` (ou `x-api-key`). As chaves vêm de `LUNA_API_KEYS` (lista separada por vírgula). `GET /health` é público.
