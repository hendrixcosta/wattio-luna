...
1. Cliente faz `POST /ask` com `Authorization: Bearer <key>` e `{ "ticket": "Não consigo gerar a fatura desse cliente." }`.
2. Middleware valida a API key (comparação em tempo constante).
3. `parseTicket` normaliza o relato (remove uma menção `@luna` inicial opcional).
...
A Luna decide o modo a partir do formato da entrada (`parseTicket`):

| Entrada | Modo | O que a Luna faz |
| --- | --- | --- |
| `TASK-12344`, `task 12344`, `task-12344`, `13344` (só a referência) | **enrich** | Recupera a task no MCP, lê comentários/anexos, investiga o código e devolve **apenas o enriquecimento** pronto para o Notion. |
| `Não consigo gerar a fatura…` (relato livre) | **enrich** | Mesmo enriquecimento, usando o texto como relato. |
| `task 12344 - qual permissão está atrelada hoje?` (referência **+ pergunta**) | **answer** | Entende o domínio, lê comentários/anexos, investiga o código e **responde objetivamente apenas a pergunta**. |

A resposta JSON inclui `mode` e `taskId` (quando reconhecido) além de `enrichment`.
...
`POST /ask` exige `Authorization: Bearer <key>` (ou `x-api-key`). As chaves vêm de `LUNA_API_KEYS` (lista separada por vírgula). `GET /health` é público. A interface de chat usa `POST /chat/stream` (mesmo núcleo, resposta em streaming NDJSON).

Rota única `POST /ask` — o comportamento é decidido pelo conteúdo do relato:
- só a referência da task (ex.: `TASK-12344`) ou um relato livre → **enriquece** o chamado;
- task seguida de uma pergunta (ex.: `task 12344 - qual permissão está atrelada hoje?`) → **responde** apenas a pergunta.
...
O corpo aceita o campo `ticket` (preferencial) ou `message` (legado). Resposta:
...
LUNA_API_KEY=<sua-key> ./scripts/ask.sh "Não estou conseguindo gerar a fatura desse cliente."
```
